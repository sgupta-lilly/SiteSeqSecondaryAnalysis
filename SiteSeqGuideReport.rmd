...
### Purpose
**ODN-seq is an in-vitro detection technique for nuclease-induced double stranded breaks (DSBs) on protease-treated genomic DNA (Tsai et al., 2014). This is an off-target nomination assay for APOC3 **
<br>
<br>
**Bioinformatic Analyses for ODN-seq pipeline (beta version)**
**report for `r guidename`**
<br>
Zipped Fastqs were trimmed for Illumina adapters and base quality assessed with FastQC using Trim Galore (v0.6.7). Reads that share the same UMI originate from the same pre-PCR molecule and are thus consolidated into a single consensus read using umitools. Sequencing reads were aligned to human reference genome (GRCh38.p14) using bowtie2 (v2.4.4) and merged using picard mergebam (v2.27.4). Bam files underwent preseq (v3.1.2) and picard collect multiplemetrics (v2.27.4) to quality check alignment. Genome coverage was assessed with bedtools genomecov (v2.30.0), bedGraphToBigWig (v377), and mosdepth coverage (v0.3.6). 
<br>
